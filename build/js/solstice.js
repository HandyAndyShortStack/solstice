...
(function() {
  var allDates, allDayLengths, colors, daysArr, height, i, index, j, len, len1, line, margin, studio, studios, svg, totalHeight, totalWidth, width, x, xAxis, y, yAxis;
  ...
  svg = d3.select('.chart').append('svg').attr('viewBox', '0 0 ' + totalWidth + ' ' + totalHeight).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
  ...
  for (index = j = 0, len1 = studios.length; j < len1; index = ++j) {
    ...
    svg.append('g').attr('data-studio-name', studio.name).append('path').datum(studio.dayLengths).attr('class', 'line').attr('d', line).style('stroke', colors(index));
  }
